...
def test_tinystories_md_latex_config_loads_expected_values():
    cfg = load_config(ROOT / "configs" / "tokenizer_bpe_tinystories_md_latex_train.yaml")

    assert cfg["run"]["output_dir"] == "artifacts/tokenizer/runs_tinystories_md_latex_train"
    assert cfg["data"]["input_paths"] == ["data/raw/TinyStoriesV2-GPT4-train.txt"]
    assert cfg["data"]["normalize"] == "none"
    assert cfg["data"]["num_workers"] == 3
    assert cfg["data"]["batch_lines"] == 1500
    assert cfg["pretokenizer"]["pattern"] == "md_latex_fast_v1"
