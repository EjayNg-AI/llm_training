...
def test_owt_md_latex_config_loads_expected_values():
    cfg = load_config(ROOT / "configs" / "tokenizer_bpe_owt_train_md_latex.yaml")

    assert cfg["run"]["output_dir"] == "artifacts/tokenizer/runs_owt_train_md_latex"
    assert cfg["data"]["input_paths"] == ["data/raw/owt_train.txt"]
